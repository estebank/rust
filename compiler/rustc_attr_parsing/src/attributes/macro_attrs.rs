...
impl<S: Stage> SingleAttributeParser<S> for CollapseDebugInfoParser {
    ...
    fn convert(cx: &mut AcceptContext<'_, '_, S>, args: &ArgParser) -> Option<AttributeKind> {
        let path = cx.expect_single_ident(args, &[sym::yes, sym::no, sym::external]);
        let info = match path {
            Ok(sym::yes) => CollapseMacroDebuginfo::Yes,
            Ok(sym::no) => CollapseMacroDebuginfo::No,
            Ok(sym::external) => CollapseMacroDebuginfo::External,
            _ => return None,
        };
