...
impl<'ast, 'ra, 'tcx> LateResolutionVisitor<'_, 'ast, 'ra, 'tcx> {
    ...
    pub(crate) fn detect_and_suggest_const_parameter_error(
            ...
            && let ItemKind::Impl(impl_) = &item.kind
        {
            ...
            let Some(idx) = current_parameter.id.and_then(|id| usage_to_pos.get(&id).copied())
            else {
                return None;
            };
            ...
            match self.resolve_qpath_anywhere(
                ...
                Ok(Some(resolve)) => {
                    if let Some(resolve) = resolve.full_res()
                        ...
                        && let Some(struct_generics) = self.r.struct_generics.get(&local_def_id)
                    {
                        ...
                        if let GenericParamKind::Const { ty, .. } = &target_param.kind
                            && let TyKind::Path(_, path) = &ty.kind
                        {
                            let full_type = path
                                .segments
                            ...
                            err.span_label(
                                target_param.span(),
                                format!(
                                    "const parameter `{}` is defined on the type",
                                    target_ident
                                ),
                            );
...
impl<'a> Visitor<'a> for ParentPathVisitor<'a> {
    fn visit_ty(&mut self, ty: &'a Ty) {
        ...
        if let TyKind::Path(_, path) = &ty.kind {
            // is this just `N`?
            if path.segments.len() == 1 && path.segments[0].ident == self.target {
                // parent is previous element in stack
                if self.stack.len() >= 2 {
                    let parent_ty = self.stack[self.stack.len() - 2];

                    if let TyKind::Path(_, parent_path) = &parent_ty.kind {
