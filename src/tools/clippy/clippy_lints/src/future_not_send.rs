...
use rustc_middle::ty::{Opaque, PredicateKind::{ImplicitSizedTrait, Trait}};
                        ...
                        |db| {
                            cx.tcx.infer_ctxt().enter(|infcx| {
                                for FulfillmentError { obligation, .. } in send_errors {
                                    ...
                                    if let ImplicitSizedTrait(trait_pred) | Trait(trait_pred, _) = obligation.predicate.kind().skip_binder() {
                                        db.note(&format!(
                                            "`{}` doesn't implement `{}`",
                                            trait_pred.self_ty(),
