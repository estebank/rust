...
impl<'f, 'tcx> Coerce<'f, 'tcx> {
    ...
    fn coerce(&self, a: Ty<'tcx>, b: Ty<'tcx>) -> CoerceResult<'tcx> {
        let a = self.shallow_resolve(a).peel_alias();
        let b = b.peel_alias();
    ...
    fn coerce_unsized(&self, source: Ty<'tcx>, target: Ty<'tcx>) -> CoerceResult<'tcx> {
        ...
        let reborrow = match (&source.kind.peel_alias(), &target.kind.peel_alias()) {
            ...
            match selcx.select(&obligation.with(trait_ref)) {
                ...
                Ok(None) => {
                    if trait_ref.def_id() == unsize_did {
                        ...
                        let self_ty = self_ty.peel_alias();
                        let unsize_ty = unsize_ty.peel_alias();
...
impl<'tcx, 'exprs, E: AsCoercionSite> CoerceMany<'tcx, 'exprs, E> {
    ...
    pub fn merged_ty(&self) -> Ty<'tcx> {
        self.final_ty.unwrap_or(self.expected_ty).peel_alias() }
    ...
    fn coerce_inner<'a>(
        ...
        debug!("coerce_inner cause={:?} expr={:?} expr_ty={:?}", cause, expression, expression_ty);
        ...
        expression_ty = expression_ty.peel_alias();
        debug!("coerce_inner expr_ty={:?}", expression_ty);
        ...
        debug!("coerce_inner result={:?}", result);
